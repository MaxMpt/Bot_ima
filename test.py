...
WIREGUARD_HOST = "103.112.69.183"
WIREGUARD_USER = "root"
WIREGUARD_KEY_PATH = None
WIREGUARD_PASSWORD = "dc8Q40d7Y6Xr"  # ← временно можно поставить пароль
...
def create_subscription(user_id: int, username: str, days: int, device: str):
    conn = sqlite3.connect(DB_NAME)
    cursor = conn.cursor()
    email = f"tg{user_id}"

    cursor.execute("""
        INSERT INTO subscriptions (user_id, username, email, device, days, status)
        VALUES (?, ?, ?, ?, ?, 'pending')
    """, (user_id, username, email, device, days))
...
async def admin_delete_client_start(callback: CallbackQuery, state: FSMContext):
    ...
    kb = InlineKeyboardMarkup(
        inline_keyboard=[[InlineKeyboardButton(text="◀️ Отмена", callback_data="admin_cancel_delete")]])
...
async def create_order(callback: CallbackQuery, state: FSMContext):
    ...
    email = create_subscription(user_id, username, days, device)
        ...
        try:
            ...
            for admin_id_str, msg_id in message_ids.items():
                try:
                    ...
                    log.info(f"✓ Удалено сообщение {msg_id} у админа {admin_id_str}")
                except TelegramBadRequest as e:
                    if "message to delete not found" in str(e).lower():
                        log.info(f"Сообщение {msg_id} уже удалено у {admin_id_str}")
                    else:
                        log.warning(f"Ошибка удаления {msg_id} у {admin_id_str}: {e}")
                except Exception as e:
                    log.warning(f"Не удалось удалить сообщение {msg_id} у {admin_id_str}: {e}")
        except Exception as e:
            log.error(f"Ошибка при разборе message_ids для {email}: {e}")
...
def generate_wireguard_config_automatic(client_name: str):
    log.info(f"=== Автоматическая генерация WireGuard для {client_name} ===")

    try:
        ssh = paramiko.SSHClient()
        ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())

        if WIREGUARD_KEY_PATH and os.path.exists(WIREGUARD_KEY_PATH):
            private_key = paramiko.RSAKey.from_private_key_file(WIREGUARD_KEY_PATH)
            ssh.connect(WIREGUARD_HOST, username=WIREGUARD_USER, pkey=private_key, timeout=25)
        else:
            ssh.connect(WIREGUARD_HOST, username=WIREGUARD_USER, password=WIREGUARD_PASSWORD, timeout=25)

        log.info("Подключение успешно")

        stdin, stdout, stderr = ssh.exec_command(f"/usr/local/bin/add-wg-client {client_name}")
        exit_status = stdout.channel.recv_exit_status()
        output = stdout.read().decode()
        error = stderr.read().decode()

        log.info(f"add-wg-client exit_status = {exit_status}")
        if error:
            log.warning(f"add-wg-client stderr: {error}")

        if exit_status != 0:
            ssh.close()
            return None

        local_path = f"./{client_name}.conf"
        sftp = ssh.open_sftp()
        remote_path = f"/root/wireguard-clients/{client_name}.conf"
        sftp.get(remote_path, local_path)
        sftp.close()
        ssh.close()

        if os.path.exists(local_path):
            log.info(f"✅ Файл успешно скачан: {local_path}")
            return local_path
        return None

    except Exception as e:
        log.error(f"Ошибка автоматической генерации WireGuard: {e}")
        return None
...
async def approve_payment(callback: CallbackQuery):
    ...
    log.info(f"=== approve_payment: device={device}, email={email} ===")

    await _delete_payment_notifications(email)

    conn = sqlite3.connect(DB_NAME, check_same_thread=False)
    cursor = conn.cursor()
    cursor.execute("UPDATE subscriptions SET status = 'active' WHERE email = ?", (email,))
    conn.commit()
    conn.close()
    ...
    if device == "router":
        try:
            await bot.send_message(
                ...
                f"Подписка на **Роутер (WireGuard)** активирована на <b>{days} дней</b>.\n\n"
                f"Сейчас генерирую конфиг...",
                parse_mode="HTML"
            )

            client_name = f"router_{user_id}"
            conf_path = generate_wireguard_config_automatic(client_name)

            if conf_path and os.path.exists(conf_path):
                with open(conf_path, "rb") as f:
                    content = f.read()

                # Нормализуем переводы строк (на всякий случай)
                content = content.replace(b'\r\n', b'\n')

                await bot.send_document(
                    user_id,
                    BufferedInputFile(content, filename=f"{client_name}.conf"),
                    caption="📄 Ваш конфиг WireGuard для роутера"
                )
                os.remove(conf_path)
                log.info(f"✅ .conf файл автоматически отправлен клиенту {user_id}")

            else:
                await bot.send_message(
                    user_id,
                    "✅ Оплата подтверждена. Конфиг будет готов в течение 1-2 минут.\nЕсли не пришёл — напишите администратору."
                )
                for admin_id in ADMIN_IDS:
                    try:
                        await bot.send_message(
                            admin_id,
                            f"📌 Нужно вручную создать WireGuard:\n\n"
                            f"Выполни на сервере:\n`add-wg-client {client_name}`\n\n"
                            f"Файл: /root/wireguard-clients/{client_name}.conf"
                        )
                    except Exception:
                        pass

        except Exception as e:
            log.error("Ошибка в ветке router: %s", e)
